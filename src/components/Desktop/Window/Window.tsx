...
const Window: React.FC<WindowProps> = ({ appID }) => {
  ...
  const { openApps, activeApp, appZIndices, appsInFullscreen, setActiveApp } =
    useAppsStore((state) => ({
  ...
  const prefersReducedMotion = useSystemStore(
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    setWindowStart({
      x: Number(appConfig.left || 0),
      y: Number(appConfig.top || 0),
    });
    setActiveApp(appID);
  ...
  const handleResizeMouseMove = (e: MouseEvent) => {
    ...
    if (resizeDirection.includes("w")) {
      newWidth -= deltaX;
      newLeft = Number(newLeft) + deltaX;
    }
    if (resizeDirection.includes("s")) newHeight += deltaY;
    if (resizeDirection.includes("n")) {
      newHeight -= deltaY;
      newTop = Number(newTop) + deltaY;
    }
